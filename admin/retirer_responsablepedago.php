     ...
     <?php                     
      for($i=0;$i<count($idProf);$i++)
      {
        ...
        for($j=0;$j<count($role);$j++)
        {
          ...
          {
        ?>
            <form method="Post" action="retirer_responsablepedago.php">
            <tr>
                    ...
                    <input type="hidden" name="id" value="<?php echo $idProf[$i];  ?>" />
                    <td> <input type="submit" class="btn btn-success"  value="retirer"
                    onclick="return confirm('Voulez vous vraiment enlever  la fonction de responsable pedagogique pour ce professeur?');"
     name="retirer">
                    </td>
                  </form>
...
<?php
  ...
  <?php
    if(isset($_REQUEST["retirer"]))
    {
      $id=$_REQUEST["id"];
      Update_responsable_pedago_prof($id);
      ?>
      <script>
        alert('La fonction de Responsable Pédagogique pour ce professeur a été enlevée');
       document.location.href="definir_responsablepedago.php";
      </script>
