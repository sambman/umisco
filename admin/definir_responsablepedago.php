    ...
    ?>
     ...
     <?php                     
      for($i=0;$i<count($idProf);$i++)
      {
        ...
        ?>  
        <tr>
            <form method="Post" action="definir_responsablepedago.php">
                    ...
                    <td> <input type="submit" class="btn btn-success"  value="definir"
                    onclick="return confirm('Voulez vous vraiment definir ce professeur en tant que responsable pedagogique');"
     name="definir">
                    </td>
          </form>
...
<?php
  ...
  <?php
    if(isset($_REQUEST["definir"]))
    {
      $id=$_REQUEST["id"];
      Update_to_responsable_pedago($id);
      ?>
      <script>
        alert('Ce professeur a ete ajouté comme Responsable Pédagogique');
       document.location.href="pedagogique.php";
      </script>
